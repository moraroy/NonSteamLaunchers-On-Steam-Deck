...
#Write to env_vars
set_launcher_vars() {
    local launcher_path1="$1"
    local launcher_path2="$2"
    local launcher_name="$3"
    local launcher_data_path1="$4"
    local launcher_data_path2="$5"
    local arg_option="$6"

    if [[ -f "$launcher_path1" ]]; then
        # Launcher is installed at path 1
        shortcutdirectory="\"$launcher_path1\" $arg_option"
        launchoptions="STEAM_COMPAT_DATA_PATH=\"${logged_in_home}/.local/share/Steam/steamapps/compatdata/${launcher_data_path1}/\" %command%"
        startingdir="\"$(dirname "$launcher_path1")\""
        ...
        shortcutdirectory="\"$launcher_path2\""
        launchoptions="STEAM_COMPAT_DATA_PATH=\"${logged_in_home}/.local/share/Steam/steamapps/compatdata/${launcher_data_path2}/\" %command%"
        startingdir="\"$(dirname "$launcher_path2")\""
        echo "export ${launcher_name}shortcutdirectory=$shortcutdirectory" >> ${logged_in_home}/.config/systemd/user/env_vars
        echo "export ${launcher_name}launchoptions=$launchoptions" >> ${logged_in_home}/.config/systemd/user/env_vars
        echo "export ${launcher_name}startingdir=$startingdir" >> ${logged_in_home}/.config/systemd/user/env_vars
...
set_launcher_vars "$amazongames_path1" "$amazongames_path2" "amazon" "NonSteamLaunchers" "AmazonGamesLauncher" ""
set_launcher_vars "$itchio_path1" "$itchio_path2" "itchio" "NonSteamLaunchers" "itchioLauncher" ""
set_launcher_vars "$legacygames_path1" "$legacygames_path2" "legacy" "NonSteamLaunchers" "LegacyGamesLauncher" ""
set_launcher_vars "$humblegames_path1" "$humblegames_path2" "humble" "NonSteamLaunchers" "HumbleGamesLauncher" ""
set_launcher_vars "$indiegala_path1" "$indiegala_path2" "indie" "NonSteamLaunchers" "IndieGalaLauncher" ""
set_launcher_vars "$rockstar_path1" "$rockstar_path2" "rockstar" "NonSteamLaunchers" "RockstarGamesLauncher" ""
set_launcher_vars "$glyph_path1" "$glyph_path2" "glyph" "NonSteamLaunchers" "GlyphLauncher" ""
